...
const BaseObject_1 = __webpack_require__(/*! ./BaseObject */ "./src/BaseObject.ts");
const PheroTrail_1 = __webpack_require__(/*! ./PheroTrail */ "./src/PheroTrail.ts");
const Point_1 = __webpack_require__(/*! ./Point */ "./src/Point.ts");
var AntState;
(function (AntState) {
    AntState[AntState["IDLE"] = 0] = "IDLE";
    AntState[AntState["PICKING"] = 1] = "PICKING";
    AntState[AntState["END"] = 2] = "END";
})(AntState = exports.AntState || (exports.AntState = {}));
class Ant extends BaseObject_1.BaseObject {
    constructor(app, location) {
        super(app, Ant.id++, location);
        this.state = AntState.PICKING;
        this.pointWeights = {};
        this.pathTaken = [];
    }
    step() {
    ...
    calculatePointWeights() {
        if (this.pathTaken.length != this.app.points.length) {
            ...
            const pointsNotVisited = this.app.points.filter(point => !this.pathTaken.includes(point.id));
            for (let i = 0; i < pointsNotVisited.length; i++) {
                ...
                const trail = PheroTrail_1.PheroTrail.getTrail(this.app, this.point, point);
    ...
    drawPointWeights(best = false) {
        if (this.state != AntState.END) {
            for (let i = 0; i < Object.keys(this.pointWeights).length; i++) {
                const point = Point_1.Point.fromID(this.app, Object.keys(this.pointWeights)[i]);
            ...
            const firstLocation = Point_1.Point.fromID(this.app, this.pathTaken[0]).location;
            this.app.ui.beginPath();
...
class App {
    ...
    setupDat() {
        ...
        this.guiObject = {
            ...
            startControl: function () {
                if (Simulation_1.Simulation.started) {
                    Simulation_1.Simulation.step();
                }
                else {
                    Simulation_1.Simulation.start();
                    app.guiControllers.focusedAnt.updateDisplay();
                }
            },
            stopControl: function () {
                if (Simulation_1.Simulation.started) {
                    Simulation_1.Simulation.started = false;
                    app.guiControllers.focusedAnt.updateDisplay();
                }
                else {
                    app.points = app.ants = [];
    ...
    step() {
        ...
        if (this.inputDriver.keyPress == " ") {
            this.points.push(new Point_1.Point(this, this.inputDriver.mousePos.clone()));
            this.bestAnt = null;
        }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.PheroTrail = void 0;
const BaseObject_1 = __webpack_require__(/*! ./BaseObject */ "./src/BaseObject.ts");
const util_1 = __webpack_require__(/*! ./util */ "./src/util.ts");
class PheroTrail extends BaseObject_1.BaseObject {
    constructor(app, point1, point2) {
        super(app, PheroTrail.id++, new util_1.Vector2(0, 0));
        this.app = app;
        this.point1 = point1;
        this.point2 = point2;
    ...
    evaporate() {
        if (this.app.guiControllers.pheromoneDecay.getValue()) {
            ...
            this.value = util_1.Utils.clamp(this.value, this.app.guiControllers.minimumPheromone.getValue(), 20);
        }
        else {
    ...
    static getTrail(app, point1, point2) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Simulation = exports.SimulationState = void 0;
const Ant_1 = __webpack_require__(/*! ./Ant */ "./src/Ant.ts");
const PheroTrail_1 = __webpack_require__(/*! ./PheroTrail */ "./src/PheroTrail.ts");
const Point_1 = __webpack_require__(/*! ./Point */ "./src/Point.ts");
const util_1 = __webpack_require__(/*! ./util */ "./src/util.ts");
var SimulationState;
(function (SimulationState) {
    SimulationState[SimulationState["PATHFINDING"] = 0] = "PATHFINDING";
    SimulationState[SimulationState["DONEFINDING"] = 1] = "DONEFINDING";
    SimulationState[SimulationState["IDLE"] = 2] = "IDLE";
...
(function (Simulation) {
    ...
    function simTicker(_ = false) {
        ...
        if (Simulation.started) {
            ...
            if (!Simulation.app.guiControllers.paused.getValue())
                step();
            setTimeout(simTicker, Simulation.app.guiControllers.simSpeed.getValue());
        }
        else {
            end();
    ...
    Simulation.simTicker = simTicker;
    function start() {
        if (Simulation.app.guiControllers.verboseConsole.getValue())
        ...
        if (Simulation.app.points.length < 3) {
            Simulation.started = false;
            return alert("Place at least 3 points");
        }
        if (Simulation.app.guiControllers.verboseConsole.getValue())
            console.log("Initializing ants");
        for (let i = 0; i < Simulation.app.guiControllers.ants.getValue(); i++) {
            const point = util_1.Utils.sample(Simulation.app.points)[0];
            const ant = new Ant_1.Ant(Simulation.app, point.location.clone());
            ant.point = point;
            ant.initialPoint = point;
            ant.pathTaken.push(point.id);
            Simulation.app.ants.push(ant);
        }
        simTicker(true);
    }
    Simulation.start = start;
    function antPickState(ant) {
        if (Simulation.app.guiControllers.verboseConsole.getValue())
            console.log("Calculating point weights");
        ant.pointWeights = {};
        ant.calculatePointWeights();
    }
    function antIdleState(ant) {
        if (Simulation.app.guiControllers.verboseConsole.getValue())
            console.log("Idle ant moving to decided point");
        const pointID = util_1.Utils.weightedRandom(ant.pointWeights);
        const point = Point_1.Point.fromID(Simulation.app, pointID);
        const trail = PheroTrail_1.PheroTrail.getTrail(Simulation.app, ant.point, point);
        if (Simulation.app.guiControllers.passiveAscend.getValue()) {
            trail.value *= 1 + util_1.Utils.normalize(Simulation.app.guiControllers.passivePheromone.getValue(), 20, 1);
        }
        else {
            trail.value = Simulation.app.guiControllers.passivePheromone.getValue();
        }
        ant.pointWeights = {};
        ant.point = point;
        ant.pathTaken.push(point.id);
        ...
        if (Simulation.app.guiControllers.turboMode.getValue())
            antPickState(ant);
        else
            ant.state = Ant_1.AntState.PICKING;
    }
    function step() {
        return __awaiter(this, void 0, void 0, function* () {
            if (Simulation.app.guiControllers.verboseConsole.getValue())
                console.log("Stepping simulation");
            switch (Simulation.simulationState) {
                case SimulationState.PATHFINDING:
                    for (let i = 0; i < Simulation.app.ants.length; i++) {
                        const ant = Simulation.app.ants[i];
                        switch (ant.state) {
                            case Ant_1.AntState.PICKING:
                                antPickState(ant);
                                break;
                            case Ant_1.AntState.IDLE:
                                antIdleState(ant);
                                break;
                            case Ant_1.AntState.END:
                                if (Simulation.app.guiControllers.verboseConsole.getValue())
                                    console.log("Ant signals done pathfinding");
                                if (!Simulation.app.bestAnt || (Simulation.app.bestAnt.pathTakenDistance > ant.pathTakenDistance))
                                    Simulation.app.bestAnt = ant;
                                Simulation.simulationState = SimulationState.DONEFINDING;
                                break;
                        }
                    }
                    break;
                case SimulationState.DONEFINDING:
                    ...
                    if (Simulation.app.bestAnt) {
                        ...
                        for (let i = 1; i < Simulation.app.bestAnt.pathTaken.length; i++) {
                            const currentPoint = Point_1.Point.fromID(Simulation.app, Simulation.app.bestAnt.pathTaken[i]);
                            const trail = PheroTrail_1.PheroTrail.getTrail(Simulation.app, previousPoint, currentPoint);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.LerpUtils = exports.Utils = exports.Vector2 = void 0;
class Vector2 {
    constructor(x, y) {
        this.x = x;
        this.y = y;
...
$(function () {
    ...
    app = new App_1.App(window.innerWidth, window.innerHeight);
    Simulation_1.Simulation.app = app;
    for (let i = 0; i < 20; i++) {
        const p = 100;
        const x = util_1.Utils.random(p, app.width - p);
        const y = util_1.Utils.random(p, app.height - p);
        app.points.push(new Point_1.Point(app, new util_1.Vector2(x, y)));
    }
});
